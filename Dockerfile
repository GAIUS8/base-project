...
COPY        . /srv/base-project

# cd /srv/airbnb와 같은 효과
WORKDIR     /srv/base-project

# 가상환경 내 패키지 설치사항 동기화
RUN         /root/.pyenv/versions/base-project/bin/pip install -r .requirements/deploy.txt
...
# collectstatic 실행
RUN         /root/.pyenv/versions/base-project/bin/python /srv/base-project/django_app/manage.py collectstatic --settings=config.settings.deploy --noinput
